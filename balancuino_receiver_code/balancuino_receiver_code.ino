...
// LED setup
#define NUM_LEDS             300
#define DATA_PIN             26    //3
#define CLOCK_PIN            4
#define LED_COLOR_ORDER      BRG   //GRB  //BGR//GBR
#define BRIGHTNESS           88    //150
...
// Potentiometers
int pot1pin = 3;    // input pin for the potentiometer 1
int pot2pin = 5;    // input pin for the potentiometer 2
int pot1val = 0;       // variable to store the value coming from potentiometer 1
int pot2val = 0;       // variable to store the value coming from potentiometer 1
...
long ATTACK_THRESHOLD=5000; // The threshold that triggers an attack
#define JOYSTICK_DEADZONE    5     // Angle to ignore
int joystickTilt = 0;              // Stores the angle of the joystick
int maxjoystickTilt=200;           // Max value used when scaling joystick angle in radio signal - Used to adjust sensitivity
...
void loop() {
    ...
    // Use pot1 reading to set attack threshold level
    ATTACK_THRESHOLD=map(pot1val,0,1024,1000,40000);
    maxjoystickTilt=map(pot2val,0,1024,25,500);
    ...
    Serial.print("maxjoystickTilt: ");
    Serial.println(maxjoystickTilt);
    */
...
void loadLevel(){
    ...
    switch(levelNumber){
        ...
        case 1:
            ...
            break;
        case 2:
            // Spawning enemies at exit every 6 seconds
            spawnPool[0].Spawn(1000, 6000, 2, 0, 0);
            break;
        case 3:
            // Lava intro
 ...
 if (!Mirf.isSending() && Mirf.dataReady())
  {
    ...
    Mirf.getData((byte *) &data);  // Read data from the sender
    
    //joystickTilt=map(data[0], 0, 255, -100, 100);
    // Doubled sensitivity
    joystickTilt=map(data[0], 0, 255, -maxjoystickTilt, maxjoystickTilt);
    joystickWobble=map(data[1],0,255,0,33000);
